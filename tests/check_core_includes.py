...
headers += [core.parent / "extensions/observed.hpp", core.parent / "dynamic/dispatch_table.hpp"]
for header in sorted(headers):
    for include in re.findall(r'^\s*#\s*include\s*[<"]([^>"]+)[>"]', header.read_text(), re.M):
        ...
        is_invocation = header.name in {"observed.hpp", "dispatch_table.hpp"} and include == "refl/runtime/invoke.hpp"
